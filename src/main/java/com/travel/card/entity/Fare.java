...
public class Fare {
    ...
    public static final float ANY_TWO_ZONES_INC_ZONE_ONE_FARE = 3.00f;
    public static final float ANY_TWO_ZONES_EXC_ZONE_ONE_FARE = 2.25f;
    public static final float THREE_PLUS_ZONES_FAIR = 3.20f;
    public static final float BUS_FARE = 1.80f;
    public static final float BASIC_TUBE_FARE = 3.20f;

    public static void validate(Transport transport, Card card) throws CustomException {
        if (transport.equals(Transport.BUS)) {
            card.validateFare(BUS_FARE);
        }
        if (transport.equals(Transport.TUBE)) {
            card.validateFare(BASIC_TUBE_FARE);
        }
    }
    ...
    public static void charge(Transport transport, Journey journey, Card card) {
        if (transport.equals(Transport.TUBE)) {
            ...
            if ( isOneZones(count) && isZoneTwo(journey)) {
                ...
                card.in(BASIC_TUBE_FARE - ANY_TWO_ZONES_INC_ZONE_ONE_FARE);
            } else if (!haveZoneOne(journey) && isTwoZones(count)) {
                card.in(BASIC_TUBE_FARE - ANY_TWO_ZONES_EXC_ZONE_ONE_FARE);
            } else if (isThreeZones(count)) {
                card.in(BASIC_TUBE_FARE - THREE_PLUS_ZONES_FAIR);
            }
    ...
    private static boolean isZoneTwo(Journey journey) {
        if (journey.getEndPoint().getZone().contains("2")
                && journey.getStartPoint().getZone().contains("2")) {
            return true;
        }
        return false;
    }
